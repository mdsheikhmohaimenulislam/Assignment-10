...
const handleFeedBack = () => {
  if (!review || rating === 0) return alert("Please provide a rating and a review.");
...
<div className="mt-10">
  ...
  <div className="flex justify-between mb-5">
    <div>
      <h3 className="text-sm md:text-xl font-bold mb-2">Leave a Review</h3>
    </div>
    ...
    {reviewsList.map((res, index) => (
      ...
      >
        ...
        <div className="flex items-center mb-1">
          {[1, 2, 3, 4, 5].map((star) => (
            <FaStar
              ...
              size={16}
              className={`${
                star <= res.rating ? "text-yellow-500" : "text-gray-300"
              }`}
            />
          ))}
        </div>
        ...
        <p className="text-xs text-gray-500 mt-1">
          By {user.displayName|| user.displayName} on {res.date || "N/A"}
        </p>
      </div>
    ))}
  </div>
